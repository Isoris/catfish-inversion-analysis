...
#
# Output (under $OUTROOT/<--out_subdir>; default subdir = "04_precomp"
# in harmonized v10 layout; was "precomp" in v9. For --mode local, pass
# --out_subdir "04_precomp_dense" so the dense outputs co-locate with
# carriers + CUSUM downstream):
#   <chr>.precomp.rds          per-window dt + (full mode) mds_mat + sim_mat
#   sim_mats/<chr>.sim_mat_nn{N}.rds   (full mode only)
#   window_dt.tsv.gz           genome-wide rollup (TSV only — NO combined RDS)
...
#   --out_subdir <name>           output subfolder under $OUTROOT
#                                 (default $SLOT_PRECOMP or "04_precomp";
#                                 pass "04_precomp_dense" for --mode local)
...
# Harmonized v10: defaults read from SLOT_* env vars when set.
OUT_LOCAL_PCA_DIR <- Sys.getenv(
  "OUT_LOCAL_PCA_DIR",
  unset = file.path(OUTROOT,
                    Sys.getenv("SLOT_LOCAL_PCA", unset = "01_local_pca")))
# Sketch dir co-locates with dense precomp by default (04_precomp_dense/sketch).
