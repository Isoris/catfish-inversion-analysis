...
#
# This is the PCA half of the former v5 monolith (was
# v5_drafts/STEP_TR_B_v5_precompute.R; the MDS + sim_mat + features half is
# now STEP_TR_C_mds_compute.R).
#
...
#   <chr>.window_pca.rds   per-chrom local PCA bundle:
...
#     $chrom, $n_windows, $n_samples, $npc, $pad, $sample_order, $pad
#
# Knobs:
...
#
# CLI overrides env vars override the hard default. NPC=4 keeps PC3/PC4
# loadings available downstream for the atlas, even though sim_mat itself
# only needs PC1.
# =============================================================================
...
message("[TR_B PCA] cohort: ", n_samp, " samples, NPC=", NPC, ", PAD=", PAD)
...
# flip decision, but each PC is flipped independently against its own
# anchor (eigenvectors are orthogonal so signs are independent).
...
for (chrom in CHROM_LIST) {
  ...
  # Per-window cohort median + per-sample-deviation |Z|. The anchor for
  # sign flips is whichever window has the largest |Z|.
