...
# =============================================================================
# STEP_ZO_E_mds_compute.R  (v10, 2026-05-16, harmonized-layout cleanup)
#
...
#   5. writes the per-chrom result to <outdir>/<chr>.mds_perchr.rds.
#
# v10: per-chrom RDS now lands DIRECTLY in <outdir> (the harmonized 03_mds/
# slot) — no <outdir>/tmp/ subdir. Downstream ZO_G discovers the MDS dir
# via env $PATH1_MDS or --mds_dir.
#
# Pipeline position:
#   ZO_C/D (per-window PCA + global IDs)  ->  ZO_E (this script)
#       ->  ZO_G (precomp + sim_mats, reads <outdir>/ per-chrom RDS)
#       ->  shared L1 / L2 detect (stripe detection on sim_mat)
#
# Inputs:
#   --rds_dir    <dir>   directory of <chr>.window_pca.rds from ZO_C/D
#   --outdir     <dir>   output dir (harmonized 03_mds/ slot)
...
#   <focal_chr>.metadata.tsv     one-row per-chrom metadata
...
# v10: per-chrom files land directly in <outdir> (the 03_mds slot). The
# legacy tmp/ subdir is no longer created. mds_dir is just outdir.
mds_dir <- outdir
...
rds_out <- file.path(mds_dir, paste0(FOCAL_CHR, ".mds_perchr.rds"))
...
fwrite(meta_row, file.path(mds_dir, paste0(FOCAL_CHR, ".metadata.tsv")), sep = "\t")
