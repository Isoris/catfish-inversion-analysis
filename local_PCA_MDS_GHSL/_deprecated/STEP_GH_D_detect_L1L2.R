#!/usr/bin/env Rscript
# =============================================================================
# STEP_GH_D_detect_L1L2.R
# =============================================================================
# GHSL stage D — D17 boundary detector wrapper. Runs the proven D17 multipass
# detector on STEP_GH_C's sim_mat and emits L1/L2 envelopes and boundaries
# in the same TSV shape as the dosage / θπ pipelines.
#
# Why a wrapper
# -------------
# STEP_GH_C ships a |Z|-threshold envelope detector as a SECONDARY layer.
# That's a 1D scan — sees peaks but doesn't see edges. The dosage and θπ
# pipelines use a much sharper detector (D17): a diagonal cross-block scan
# that asks "are samples on the LEFT dissimilar from samples on the RIGHT
# at this position?", which is exactly what an inversion edge looks like.
# D17 ships with a validated growing-W validator, adaptive thresholding,
# Ward-D2 segment-internal-homogeneity stratification, and
# STABLE_BLUE / EDGE / FAKE / DEMOTED status labels.
#
...
#
# This wrapper:
#  1. Reads <chr>.ghsl_localpca.rds (STEP_GH_C output)
...
#
# Output TSVs (consumed natively by STEP_GH_E_atlas_json.R via the
# --d17_l1_env / --d17_l2_env / --d17_l1_bnd / --d17_l2_bnd flags):
...
#
# Asymmetry vs θπ: a parallel wrapper exists for θπ (STEP_TR_C_theta_d17_wrapper.R).
...
#    (geometry-based edge detection), STEP_GH_B PASS-runs are PRIMARY
#    biologically (calibrated on real signal). They answer different
#    questions — keep both, don't reconcile, the atlas overlays both.
#    STEP_GH_C's |Z|-threshold secondaries become tertiary when D17 is
#    available; in practice the atlas can render any subset based on
#    what's present.
#  - For θπ: D17 envelopes (the parallel wrapper) PLUS theta_pi's
...
# Usage
# -----
#   Rscript STEP_GH_D_detect_L1L2.R \
#     --localpca       <localpca_dir>/<CHR>.ghsl_localpca.rds \
...
if (is.null(LOCALPCA) || !file.exists(LOCALPCA)) {
  stop("[GH_D] FATAL: --localpca <file>.ghsl_localpca.rds is required")
}
if (is.null(D17_L1_SCRIPT) || !file.exists(D17_L1_SCRIPT)) {
  stop("[GH_D] FATAL: --d17_l1_script must point at STEP_D17_multipass_L1_only_v7.R")
}
if (is.null(D17_L2_SCRIPT) || !file.exists(D17_L2_SCRIPT)) {
  stop("[GH_D] FATAL: --d17_l2_script must point at STEP_D17_multipass_L2_v8.R")
}

dir.create(OUTDIR, recursive = TRUE, showWarnings = FALSE)

message("================================================================")
message("[GH_D] GHSL local-PCA → D17 boundary detector wrapper")
message("================================================================")
message("[GH_D] localpca RDS : ", LOCALPCA)
message("[GH_D] D17 L1 script: ", D17_L1_SCRIPT)
message("[GH_D] D17 L2 script: ", D17_L2_SCRIPT)
message("[GH_D] outdir       : ", OUTDIR)

# ---- Load STEP_GH_C output --------------------------------------------------
message("[GH_D] Reading STEP_GH_C local-PCA RDS")
lp <- readRDS(LOCALPCA)
chrom <- if (!is.null(CHR_LABEL)) CHR_LABEL else lp$chrom
n_win <- lp$n_windows %||% nrow(lp$window_info)

if (is.null(chrom) || is.na(chrom)) {
  stop("[GH_D] FATAL: cannot determine chrom (not in RDS, not in --chr)")
}
message("[GH_D] chrom: ", chrom, "   n_windows: ", n_win)

# Sanity checks
if (is.null(lp$window_info) || nrow(lp$window_info) != n_win) {
  stop(sprintf("[GH_D] FATAL: window_info shape mismatch (n=%d, want %d)",
               nrow(lp$window_info %||% data.frame()), n_win))
}
if (is.null(lp$sim_mat) || !is.matrix(lp$sim_mat)) {
  stop("[GH_D] FATAL: lp$sim_mat is not a matrix (was STEP_GH_C output corrupted?)")
}
if (nrow(lp$sim_mat) != n_win || ncol(lp$sim_mat) != n_win) {
  stop(sprintf("[GH_D] FATAL: sim_mat is %dx%d but n_windows=%d",
...
message("[GH_D] Building D17-compatible precomp")
...
precomp_obj <- list(
  ...
  source      = "STEP_GH_C local-PCA → STEP_GH_D D17 wrapper",
  generator   = "STEP_GH_D_detect_L1L2.R"
)
...
message("[GH_D] Writing temp precomp: ", precomp_temp)
saveRDS(precomp_obj, precomp_temp)

message("[GH_D] Writing temp sim_mat: ", sim_mat_temp)
...
message("[GH_D] L1 cmd: ", l1_cmd)
l1_status <- system(l1_cmd)
if (l1_status != 0) {
  stop("[GH_D] FATAL: D17 L1 failed with exit code ", l1_status)
}
...
message("[GH_D] L2 cmd: ", l2_cmd)
l2_status <- system(l2_cmd)
if (l2_status != 0) {
  stop("[GH_D] FATAL: D17 L2 failed with exit code ", l2_status)
}
...
message("[GH_D] D17 L2 produced: ", n_l2_env, " L2 envelopes, ",
...
rename_outputs <- function(orig_path, ghsl_suffix) {
  ...
  if (orig_path != new_path) {
    ...
    message("[GH_D]   renamed: ", basename(orig_path),
            " → ", basename(new_path))
  }
}

message("\n[GH_D] Renaming outputs to GHSL namespace")
...
if (!KEEP_TEMP) {
  message("[GH_D] Cleaning temp dir: ", temp_dir)
  unlink(temp_dir, recursive = TRUE)
} else {
  message("[GH_D] Keeping temp files (--keep_temp): ", temp_dir)
}
